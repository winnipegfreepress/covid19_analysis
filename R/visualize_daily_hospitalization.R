...
p_daily_hospitalization <- ggplot(wfp_daily_hospitalization_totals) +
  ...
  geom_text(data=wfp_daily_hospitalization_totals %>% filter(date == max(date)),
            stat="identity",
            aes(x=date - 20, y=y_max - 10, label=paste(total_hospital, " in hospital", sep="")),
  ...
  geom_text(data=wfp_daily_hospitalization_totals %>% filter(date == max(date)),
            stat="identity",
            aes(x=date - 20, y=y_max - 35, label=paste(active_hospital, " active in hospital", sep="")),
  ...
  geom_text(data=wfp_daily_hospitalization_totals %>% filter(date == max(date)),
            stat="identity",
            aes(x=date - 20, y_max - 60, label=paste(total_icu, " in ICU", sep="")),
  ...
  geom_text(data=wfp_daily_hospitalization_totals %>% filter(date == max(date)),
            stat="identity",
            aes(x=date - 20, y_max - 85, label=paste(active_icu, " active in ICU", sep="")),
