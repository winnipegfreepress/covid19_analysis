...
Manitoba_COVID_19_Vaccine_Uptake_by_District__1stdose <- Manitoba_COVID_19_Vaccine_Uptake_by_District %>%
  arrange(desc(uptake_1)) %>%
  ggplot() +
  aes(x=reorder(rhadname, uptake_1), y=uptake_1, fill=rha) +
  geom_col(aes(x=reorder(rhadname, uptake_1), y=uptake_1, fill=rha)) +
...
wfp_covid_vaccine_uptake_1stdose_all <- prepare_plot(Manitoba_COVID_19_Vaccine_Uptake_by_District__1stdose)
ggsave_pngpdf(wfp_covid_vaccine_uptake_1stdose_all, "wfp_covid_vaccine_uptake_1stdose_all", width_var=8.66, height_var=21, dpi_var=300, scale_var=1, units_var="in")
